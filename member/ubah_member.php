...
$level		= $_POST['level'];
	...
	if (count($pesanError)>=1 ){
		...
		$mySql	= "UPDATE member SET email='$email' $sqlpassword, nama='$nama', alamat='$alamat', telepon='$telepon', level='$level' WHERE idpemesan ='".$_POST['idpemesan']."'";
...
$myData= mysql_fetch_array($myQry);
	...
	$datanama	= isset($_POST['alamat']) ? $_POST['alamat'] : $myData['alamat'];
	$datanama	= isset($_POST['telepon']) ? $_POST['telepon'] : $myData['telepon'];
    ...
    <input type="hidden" name='idpemesan' size=15  value= "<?php echo $dataKode; ?>" >
      ...
      <input name='passwords' type="text"  size=20>
       <input name='passlama' type="hidden" value="<?php echo $myData['passwords']; ?>" size=20></td>
		...
		<tr>
            <td>Nama</td>
            <td><input type="text" name="nama" size="20" value="<?php echo $myData['nama']; ?>">
            </td> 
    	</tr>
        <tr>
        <td>Alamat</td>
        <td><textarea name="alamat"><?php echo $myData['alamat']; ?></textarea></td>
        </tr>
        <tr>
        <td>Telepon</td>
        <td><input type="text" name="telepon" value="<?php echo $myData['telepon'];?>" />
        <input type="hidden" name="level" value="member" size="20">
        </td>
        </tr>
		        <tr>
            <td></td><td>
